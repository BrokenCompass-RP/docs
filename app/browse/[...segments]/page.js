...
export default async function BrowseFolderPage({ params, searchParams }) {
  ...
  return <main className="shell" id="page-top">
    <nav className="topbar" aria-label="Breadcrumb">
      <div className="topbar-breadcrumb"><Link href="/">Knowledge</Link>{folder.segments.map((segment, index) => {
      ...
      <div className="topbar-actions"><Link href="/search">Search</Link><AuthControls authorization={authorization} /></div>
    </nav>
    ...
    <ReaderUtilityFooter pageLabel={folder.name} identity={authorization.identity} projection={projection} returnTo={returnTo} />
  </main>;
}

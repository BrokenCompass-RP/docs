...
export class PostgresDraftRepository {
  ...
  async readBrowsePath(slug) {
    const result = await this.pool.query(`SELECT dr.browse_path FROM document_drafts dr JOIN documents d ON d.id=dr.document_id WHERE d.slug=$1`, [slug]);
    return result.rows[0]?.browse_path ?? null;
  }
  ...
  async save(slug, source, options = {}) {
    ...
    const now = new Date();
    await this.pool.query(`INSERT INTO document_drafts(document_id, canonical_markdown, author_identity, base_published_version_id, browse_path, created_at, updated_at)
      VALUES($1,$2,$3,$4,$5,$6,$6)
      ON CONFLICT(document_id) DO UPDATE SET canonical_markdown=EXCLUDED.canonical_markdown, author_identity=EXCLUDED.author_identity, browse_path=EXCLUDED.browse_path, updated_at=EXCLUDED.updated_at`,
      [result.rows[0].id, source, options.authorIdentity ?? null, result.rows[0].current_published_version_id, options.browsePath ?? null, now]);
  }
...
export class PostgresVersionRepository {
  ...
  async ensureInitialized(slug, canonicalMarkdown) {
    ...
    try {
      ...
      await client.query(`INSERT INTO documents(id,slug,title,description,default_visibility,browse_path,route_path,historical_first_published_at,imported_at,created_at,updated_at)
        VALUES($1,$2,$3,$4,$5,$6,$7,NULL,$8,$8,$8) ON CONFLICT(slug) DO NOTHING`,
        [definition.id, slug, parsed.frontmatter.title, parsed.frontmatter.description ?? "", parsed.frontmatter.default_visibility, definition.browsePath, definition.url, now]);
  ...
  async publishDraftTransaction(slug, publishedBy, validate) {
    ...
    try {
      ...
      const parsed = validate(draft.canonical_markdown);
      ...
      await client.query(`UPDATE documents SET current_published_version_id=$1,updated_at=$2,historical_first_published_at=CASE WHEN current_published_version_id IS NULL THEN $2 ELSE historical_first_published_at END,title=$4,description=$5,default_visibility=$6,browse_path=COALESCE($7,browse_path) WHERE id=$3`,
        [id, now, document.id, parsed.frontmatter.title, parsed.frontmatter.description ?? "", parsed.frontmatter.default_visibility, draft.browse_path]);
  ...
  async recover(slug, versionId, options = {}) {
    ...
    try {
      ...
      const parsed = parseCanonicalMarkdown(historical.canonical_markdown);
      ...
      await client.query(`UPDATE documents SET current_published_version_id=$1,updated_at=$2,title=$4,description=$5,default_visibility=$6 WHERE id=$3`, [id, now, document.id, parsed.frontmatter.title, parsed.frontmatter.description ?? "", parsed.frontmatter.default_visibility]);
      await client.query("COMMIT");
      return this.getVersion(slug, versionLabel(versionNumber));
    } catch (error) { await client.query("ROLLBACK"); throw error; }
    finally { client.release(); }

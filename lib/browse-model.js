...
export function findBrowseDocumentPath(browse, slug) {
  const visit = (folders, ancestors = []) => {
    for (const folder of folders) {
      const path = [...ancestors, folder];
      const document = folder.documents.find((entry) => entry.slug === slug);
      if (document) return { folders: path, document };
      const nested = visit(folder.children, path);
      if (nested) return nested;
    }
    return null;
  };
  return visit(browse.folders);
}

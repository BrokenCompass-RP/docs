...
export class PostgresAuthRepository {
  ...
  async establishDiscordSession(input) {
    ...
    try {
      ...
      await client.query(`INSERT INTO actor_external_identities(provider,provider_user_id,actor_id,username,display_name,updated_at)
        VALUES('discord',$1,$2,$3,$4,$5)
        ON CONFLICT(provider,provider_user_id) DO UPDATE SET username=EXCLUDED.username,display_name=EXCLUDED.display_name,updated_at=EXCLUDED.updated_at`,
        [input.discordUserId, actorId, input.username ?? null, input.displayName ?? null, now]);
      ...
      // Data hygiene is best effort and must not undo an established login.
      try { await client.query("DELETE FROM authorization_sessions WHERE expires_at <= now()"); } catch {}

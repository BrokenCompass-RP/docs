...
export function createReviewService({ repository }) {
  return {
    async createFlag(input, identity) {
      ...
      return repository.createFlag({
        ...
        reporterIdentity: identity === "public" ? null : identity
      });
    },
    ...
    async addComment(slug, flagId, body, identity) {
      if (!hasCapability(identity, CAPABILITIES.MANAGE_REVIEWS)) throw new Error("Review access is not authorized");
      return repository.addComment(slug, flagId, identity, boundedText(body, "Comment", 2000));
    },

    async resolve(slug, flagId, identity, versionAtResolution) {
      if (!hasCapability(identity, CAPABILITIES.MANAGE_REVIEWS)) throw new Error("Review access is not authorized");
      return repository.resolve(slug, flagId, identity, versionAtResolution);
    }
  };
}

...
function renderer(assetUrl) {
  const instance = new MarkdownIt({ html: false, linkify: true, typographer: true });
  const fallback = instance.renderer.rules.image;
  instance.renderer.rules.image = (tokens, index, options, env, self) => {
    const token = tokens[index];
    const sourceIndex = token.attrIndex("src");
    const source = sourceIndex >= 0 ? token.attrs[sourceIndex][1] : "";
    const match = /^asset:([0-9a-f-]{36})$/i.exec(source);
    if (!match) return fallback ? fallback(tokens, index, options, env, self) : self.renderToken(tokens, index, options);
    token.attrs[sourceIndex][1] = assetUrl ? assetUrl(match[1]) : `/api/assets/${match[1]}`;
    token.attrSet("loading", "lazy");
    const caption = token.attrGet("title");
    if (caption) token.attrs.splice(token.attrIndex("title"), 1);
    const image = fallback ? fallback(tokens, index, options, env, self) : self.renderToken(tokens, index, options);
    return caption ? `<figure class="knowledge-image">${image}<figcaption>${instance.utils.escapeHtml(caption)}</figcaption></figure>` : image;
  };
  return instance;
}
...
export function renderProjection(document, identity, options = {}) {
  const authorizedMarkdown = projectDocument(document, identity)
    .map((block) => block.markdown)
    .join("\n\n");
  return renderer(options.assetUrl).render(authorizedMarkdown);
}
